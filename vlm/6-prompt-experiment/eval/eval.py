...
def calculate_group_accuracy(df, group_col, col1, col2):
    ...
    df_valid = df[df[col2] != -1].copy()
    
    if len(df_valid) == 0:
        print(f"Warning: No valid predictions found for columns {col1} and {col2}")
        return pd.DataFrame(columns=[group_col, 'accuracy'])
    
    # Check if columns exist
    if col1 not in df_valid.columns:
        print(f"Error: Column '{col1}' not found in DataFrame")
        print(f"Available columns: {list(df_valid.columns)}")
        return pd.DataFrame(columns=[group_col, 'accuracy'])
    
    if col2 not in df_valid.columns:
    ...
    def calc_accuracy(group):
        return (group[col1] == group[col2]).mean()
    
    accuracy_by_group = df_valid.groupby(group_col).apply(calc_accuracy).reset_index()
    accuracy_by_group.columns = [group_col, 'accuracy']
...
def calc_acc(df, gt_name, predict_name):
    ...
    """
    print(f"Calculating accuracy for columns: {gt_name} vs {predict_name}")
    print(f"DataFrame shape: {df.shape}")
    print(f"Available columns: {list(df.columns)}")
    
    accuracy_df = calculate_group_accuracy(df, "prompt", gt_name, predict_name)
    
    if len(accuracy_df) == 0 or 'accuracy' not in accuracy_df.columns:
        print(f"Error: Could not calculate accuracy for {gt_name} vs {predict_name}")
        return 0, 0, pd.DataFrame()
    
    mean_accuracy = round(accuracy_df['accuracy'].mean() * 100, 2)
    std_accuracy = round(np.std(np.array(accuracy_df['accuracy']), ddof=1) * 100, 2)

    return mean_accuracy, std_accuracy, accuracy_df
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_predictions', type=str, required=False,
                        default='vlm/results/gpt_4o_caption')
    parser.add_argument('--input_file', type=str, default=None, help='Path to a specific predictions CSV file')
    parser.add_argument('--language', type=str, default=None, help='Specific language to evaluate (e.g., "hi")')
    ...
    if args.input_file and args.language:
        print("----Language: {}-----".format(args.language))
        if os.path.exists(args.input_file):
            df_inference = pd.read_csv(args.input_file)
            root = os.path.dirname(args.input_file)  # Define root for output_path
        else:
        ...
        df_inference = df_inference[["Meme ID", "prompt", "response", "answer", "hate_prediction"]]
        
        output_path = os.path.join(root, "processed_responses_" + args.language + ".csv")
        ...
        print(f"Number of invalid responses: {n_invalid}")
        
        # Accuracy - only for the specified language
        print(f"------EVAL LANG {args.language}------")
        gt_col = MAPPING[args.language]
        
        # Check if the ground truth column exists
        if gt_col not in df_inference.columns:
            print(f"Error: Ground truth column '{gt_col}' not found in merged DataFrame")
            print(f"Available columns: {list(df_inference.columns)}")
            sys.exit(1)
        
        accuracy, std, df_acc = calc_acc(df_inference, gt_col, "hate_prediction")
        print("Accuracy: {}\nStandard Deviation: {}".format(accuracy, std))
        
    else:
        # Original folder walk
            ...
            for file in files:
                for language in LANGUAGES:
                    ...
                    if os.path.exists(folder_path):
                        ...
                        output_path = os.path.join(root, "processed_responses_" + language + ".csv")
                        df_inference.to_csv(output_path, index=False)
                        ...
                        for language_eval in LANGUAGES:
                            ...
                            gt_col = MAPPING[language_eval]
                            
                            if gt_col not in df_inference.columns:
                                print(f"Warning: Ground truth column '{gt_col}' not found, skipping...")
                                continue
